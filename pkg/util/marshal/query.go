...
func NewStream(s logproto.Stream) (loghttp.Stream, error) {
	labels, err := NewLabelSetFromGroups(s.Labels)
	if err != nil {
...
func encodeGroupedLabels(groupedLabels logproto.GroupedLabels, s *jsoniter.Stream) error {
	groupsMap := logproto.FromGroupedLabelsToMap(groupedLabels)
	for group, lbls := range groupsMap {
		if group == "" {
			return fmt.Errorf("grouped labels must have a group name")
		}
...
func encodeStream(stream logproto.Stream, s *jsoniter.Stream, encodeFlags ...loghttp.EncodingFlag) error {
	...
	if loghttp.EncodingFlagIsSet(encodeFlags, loghttp.FlagGroupLabels) {
		if err := encodeGroupedLabels(stream.Labels, s); err != nil {
			return err
		}
	} else {
		// If ungrouped, we write all labels from different groups one after the other
		if err := encodeLabels(stream.Labels.Stream, s); err != nil {
			return err
		}
		if err := encodeLabels(stream.Labels.StructuredMetadata, s); err != nil {
			return err
		}
		if err := encodeLabels(stream.Labels.Parsed, s); err != nil {
			return err
		}
	}
